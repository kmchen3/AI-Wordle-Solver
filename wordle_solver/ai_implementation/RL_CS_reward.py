...
def train_actor_critic(all_words, episodes=200, lr=1e-3, gamma=0.99, print_interval=100):
    ...
    for ep in range(1, episodes + 1):
        ...
        for step in range(6):
            ...
            # feedback reward
            r = 0.2 * green + 0.1 * yellow

            # word elimination 
            before = len(possible) # words possible before making guess
            possible = filter_words(possible, guess, fb)
            after = len(possible) # words possible after checking feedback
            # eliminated = before - after # words removed from list
            if before > 0: # if there were words
                # r += 0.5 * ((eliminated / before) ** 2) 
                r += math.log(before + 1) - math.log(after + 1)

            # If solved, reward based on how fast it was solved
            if fb == "ggggg":
                r += 1.0 + ((6 - step) ** 2) / 36.0  # quadratic growth [0, 1]
...
def evaluate_policy_subset(actor, all_words, word2idx, N=100):
    """
    Evaluates the policy on a random subset of size N.
    """
    V = len(all_words)
    subset = random.sample(all_words, N)
    tries = Counter()
    
    for target in tqdm(subset, desc=f"Eval on {N} words"):
        possible = all_words.copy()
        for t in range(1, 7):
            state = compute_state(possible)
            mask = torch.zeros(V, dtype=torch.bool)
    ...
    total = sum(tries.values())
    print(f"\nEvaluation on {N} random words:")
    for k in sorted(tries):
        print(f"  Solved in {k}: {tries[k]} ({tries[k]/total:.2%})")


if __name__ == "__main__":
    words = load_words("prev_wordle_words.json")
    actor_model, critic_model = train_actor_critic(words)
    word2idx = {w: i for i, w in enumerate(words)}
    evaluate_policy_subset(actor_model, words, word2idx)